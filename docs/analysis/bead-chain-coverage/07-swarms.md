...
> Seeded stub. Fill from `_template.md`. Owner bead: bead_chain-jmo.
...
| Primary modules  | `lifecycle.py`, `register_callbacks.py`                                      |
| Status           | `not-started`                                  |
...
_TODO (bead_chain-jmo): summarize the bd 1.0.4 feature surface for this area, citing
`field-guide-07-swarms.html` § "<section>"._

## 2. LEVERAGED — what bead-chain actually uses

_TODO (bead_chain-jmo): what bead-chain consumes, with `file:line` citations. State
