...
> Seeded stub. Fill from `_template.md`. Owner bead: bead_chain-5xh.
...
| Primary modules  | `lifecycle.py` (`rollup_completed_epics`, `_has_fan_out_gate_issue`)                                      |
| Status           | `not-started`                                  |
...
_TODO (bead_chain-5xh): summarize the bd 1.0.4 feature surface for this area, citing
`field-guide-05-formulas-and-molecules.html` § "<section>"._

## 2. LEVERAGED — what bead-chain actually uses

_TODO (bead_chain-5xh): what bead-chain consumes, with `file:line` citations. State
