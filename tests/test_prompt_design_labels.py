"""Unit tests for design + labels rendering in the goal prompt.
...
field, but :func:`prompt.format_bead_as_goal` historically read neither
...
from code_puppy.plugins.bead_chain import prompt  # noqa: E402
...
# --------------------------------------------------------------------------
# format_bead_as_goal: labels present case
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# format_bead_as_goal: labels absent / empty cases -> prompt unchanged
# --------------------------------------------------------------------------
...
def test_empty_list_labels_no_line():
    out = prompt.format_bead_as_goal(_base_bead(labels=[]))
    assert "- Labels:" not in out
...
def test_non_list_labels_no_line():
    out = prompt.format_bead_as_goal(_base_bead(labels="bead-chain"))
    assert "- Labels:" not in out
...
# --------------------------------------------------------------------------
# format_bead_as_goal: design present case
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# format_bead_as_goal: design absent / empty cases -> prompt unchanged
# --------------------------------------------------------------------------
...
def test_empty_string_design_no_heading():
    out = prompt.format_bead_as_goal(_base_bead(design=""))
    assert _DESIGN_HEADING not in out
...
def test_non_string_design_no_heading():
    out = prompt.format_bead_as_goal(_base_bead(design={"k": "v"}))
    assert _DESIGN_HEADING not in out
