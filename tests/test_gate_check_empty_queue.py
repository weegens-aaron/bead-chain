...
import pytest
from code_puppy.plugins.bead_chain import lifecycle, state
from code_puppy.plugins.bead_chain.beads import BeadsError
...
def test_empty_queue_runs_gate_check_once_then_stops(monkeypatch):
    ...
    monkeypatch.setattr(lifecycle, "check_gates", fake_check_gates)
...
def test_gate_check_failure_soft_fails_and_drains(monkeypatch):
    ...
    monkeypatch.setattr(lifecycle, "check_gates", boom)
    calls = _stub_activation_surface(monkeypatch)
...
def test_probe_returns_true_only_when_resolved_positive(monkeypatch):
    ...
    monkeypatch.setattr(
        lifecycle, "check_gates", lambda: {"resolved": 2, "escalated": 0}
    )
    assert lifecycle.probe_resolved_gates() is True

    monkeypatch.setattr(
        lifecycle, "check_gates", lambda: {"resolved": 0, "escalated": 3}
    )
