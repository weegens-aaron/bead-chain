...
from code_puppy.plugins.bead_chain import beads, prompt  # noqa: E402
...
def test_block_caps_entry_count():
    ...
    assert len(bullets) == prompt._MEMORY_DIGEST_MAX_ENTRIES
...
def test_block_truncates_long_insight():
    ...
    assert "x" * (prompt._MEMORY_EXCERPT_LIMIT + 50) not in out
    assert "…" in out
...
def test_fetch_soft_fails_to_empty_on_beads_error(monkeypatch):
    ...
    monkeypatch.setattr(prompt, "memories", boom)
    assert prompt._fetch_memory_digest() == {}


def test_fetch_passes_through_memories(monkeypatch):
    monkeypatch.setattr(prompt, "memories", lambda: {"k": "v"})
