...
# br variant: default to the beads_rust binary (``br``) instead of ``bd``.
# A ``BEADS_BIN`` override still wins (see :func:`_bd_bin`).
DEFAULT_BD_BIN = "br"
...
# One-line change, by design — :func:`is_excluded_type` filters this set
# out client-side. br variant (spike bead_chain-5d3): ``br`` has no
# ``--exclude-type`` query flag, so that client-side check is the *sole*
# filter (the bd variant also passed the flag server-side via the now-unused
# :func:`_exclude_type_arg`). DRY.
...
def is_excluded_type(bead: dict[str, Any] | None) -> bool:
    ...
    br variant: the **sole** epic/container filter. ``br`` has no
    ``--exclude-type`` query flag (spike bead_chain-5d3), so unlike the
    bd variant — where this was a client-side companion to a server-side
    flag — every queue read here relies entirely on this check to keep
    epics/milestones/gates/molecules out of the drivable-work frontier.
    It must never be weakened or removed.

    The check is case-insensitive on ``issue_type`` so an upstream
    beads that suddenly emits ``"Epic"`` instead of ``"epic"`` doesn't
...
def _exclude_type_arg() -> str:
    ...
    .. note:: **Unused in the br variant.** ``br`` has no
       ``--exclude-type`` query flag (spike bead_chain-5d3), so no caller
       passes this arg — epic filtering is done entirely client-side via
       :func:`is_excluded_type`. The helper is retained for parity with
       the bd variant and in case a future ``br`` adds the flag.
    """
...
def _parse_json_list(raw: str, context: str) -> list[Any]:
    """Parse JSON from br output, expecting a list (or a list envelope).
    ...
    br variant envelope handling
    ----------------------------
    ``br ready --json`` (and ``br ready --type/--parent``) emits a bare
    top-level JSON **array**, just like ``bd``. But ``br list --json``
    wraps its results in an **object** envelope ``{"issues": [...],
    "total": N, ...}`` (verified against ``br 0.2.15``, spike
    bead_chain-5d3). To keep one parser for both command families, we
    transparently unwrap a single-key-ish object whose ``"issues"`` value
    is a list before applying the list check. A bare array passes straight
    through unchanged.

    Args:
        raw: Raw stdout from a br command.
        context: Human-readable command description for error messages,
                 e.g. "br ready --json" or "br list --status=in_progress".

    Raises:
        BeadsError: On non-JSON output or a payload that is neither a list
            nor an ``{"issues": [...]}`` envelope.
    """
    try:
    ...
    # br ``list --json`` envelope: unwrap ``{"issues": [...]}`` to the bare
    # array the rest of the pipeline expects. Only unwrap when ``issues``
    # is actually a list — otherwise fall through to the list check below
    # so a malformed payload still raises a clear error.
    if isinstance(items, dict) and isinstance(items.get("issues"), list):
        return items["issues"]
