...
def has_epic_in_progress() -> bool:
    ...
    Wraps ``br list --type=epic --status=in_progress --json``. Used to
    decide whether bead-chain needs to start a new epic or if one is
    already being tracked as active.

    br variant envelope (spike bead_chain-5d3): ``br list --json`` returns
    an object ``{"issues": [...], ...}`` rather than a bare array. We
    unwrap that ``"issues"`` list before counting — without this the check
    would silently return ``False`` forever and bead-chain would keep
    starting fresh epics on top of an already-active one.
    """
    ...
    # br ``list --json`` envelope: unwrap ``{"issues": [...]}`` to the list.
