...
# be exactly ``bd`` or ``br``. br variant (spike bead_chain-5d3): an agent
# running ``BEADS_BIN=br`` would shell out ``br close``/``br update`` — if
# the guard only matched ``bd`` those would bypass the LLM-judge close
# guard *undetected*. This is a SAFETY widening, not a cosmetic one.
_BD_INVOCATION = r"(?:\S*/)?(?:bd|br)"

# Match ``bd close [...]`` / ``br close [...]`` — any subcommand-style
# invocation of close regardless of trailing flags or bead id.
_BD_CLOSE_RE = re.compile(
    rf"{_COMMAND_BOUNDARY}{_BD_INVOCATION}\s+close\b", re.MULTILINE
)

# Match ``bd update <id> --status=closed`` / ``br update <id>
# --status=closed`` (or ``--status closed``). We restrict the gap between
# ``update`` and ``--status`` to the same command (no shell separators) so
# a later chained command doesn't get blamed on the earlier
# ``update --claim``.
...
def detect_premature_close(command: str) -> CloseGuardMatch | None:
    ...
    # possibly invoke bd/br. ``"bd"``/``"br"`` appear in plenty of
    # unrelated strings, but it's a small enough set to be worth the
    # savings. br variant: ``"br"`` does NOT contain ``"bd"``, so we MUST
    # check both substrings here — a pre-filter that only looked for
    # ``"bd"`` would short-circuit and let every ``br close`` through
    # undetected (spike bead_chain-5d3).
    if "bd" not in command and "br" not in command:
        return None
    ...
    if _BD_CLOSE_RE.search(scannable):
        return CloseGuardMatch(
            pattern_name="bd/br close",
            description="Direct `bd close`/`br close` bypasses the LLM judges.",
        )
    if _BD_UPDATE_STATUS_CLOSED_RE.search(scannable):
        return CloseGuardMatch(
            pattern_name="bd/br update --status=closed",
